...
function SessionRow({ item, language }: { item: any; language: 'ko' | 'en' }) {
  ...
  const unavailable = new Set(item.unsupportedFields ?? []);
  const missing = language === 'ko' ? '제공되지 않음' : 'Not provided';
  return <tr className="group transition-colors hover:bg-secondary/35" data-testid={`row-session-${item.id}`}><td className="px-5 py-4"><div className="text-xs font-semibold text-slate-200">{item.project}</div><div className="mt-1 max-w-[260px] truncate text-[11px] text-muted-foreground">{item.task}</div><div className="mono mt-1 text-[10px] text-muted-foreground">{new Date(item.date).toLocaleString(language === 'ko' ? 'ko-KR' : 'en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</div></td><td className="px-5 py-4"><span className="mono text-xs text-slate-300">{unavailable.has('model') ? missing : item.model}</span></td><td className="px-5 py-4"><div className="mono text-xs text-slate-200">{unavailable.has('actualTokens') ? missing : formatTokens(item.actualTokens)}</div><div className="mt-1 text-[10px] text-muted-foreground">{formatTokens(item.predictedTokens.min)}–{formatTokens(item.predictedTokens.max)} {language === 'ko' ? '예측' : 'pred.'}</div></td><td className="px-5 py-4 mono text-xs text-slate-200">{unavailable.has('cost') ? missing : formatMoney(item.cost)}</td><td className="px-5 py-4"><div className="flex items-center gap-2"><div className="h-1.5 w-16 overflow-hidden rounded-full bg-secondary"><div className="h-full rounded-full bg-primary" style={{ width: `${item.complexity}%` }} /></div><span className="mono text-[10px] text-muted-foreground">{item.complexity}</span></div></td><td className="px-5 py-4"><span className={`inline-flex rounded-md border px-2 py-1 text-[10px] ${tone === 'accent' ? 'border-accent/25 bg-accent/10 text-accent' : tone === 'danger' ? 'border-destructive/25 bg-destructive/10 text-destructive' : 'border-primary/25 bg-primary/10 text-primary'}`}>{language === 'ko' ? (rec.includes('upgrade') ? '업그레이드' : rec.includes('watch') ? '주의' : rec.includes('unavailable') ? '제공되지 않음' : '유지') : item.recommendation}</span></td></tr>;
}
function MobileSessionRow({ item, language }: { item: any; language: 'ko' | 'en' }) {
  const unavailable = new Set(item.unsupportedFields ?? []);
  const missing = language === 'ko' ? '제공되지 않음' : 'Not provided';
  return <div className="p-4" data-testid={`card-session-${item.id}`}><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold text-slate-200">{item.project}</div><div className="mt-1 text-xs text-muted-foreground">{item.task}</div></div><span className="mono text-[10px] text-primary">{item.recommendation}</span></div><div className="mt-4 grid grid-cols-3 gap-2"><div><div className="mono text-[9px] uppercase text-muted-foreground">{language === 'ko' ? '토큰' : 'tokens'}</div><div className="mono mt-1 text-xs">{unavailable.has('actualTokens') ? missing : formatTokens(item.actualTokens)}</div></div><div><div className="mono text-[9px] uppercase text-muted-foreground">{language === 'ko' ? '비용' : 'cost'}</div><div className="mono mt-1 text-xs">{unavailable.has('cost') ? missing : formatMoney(item.cost)}</div></div><div><div className="mono text-[9px] uppercase text-muted-foreground">{language === 'ko' ? '복잡도' : 'complexity'}</div><div className="mono mt-1 text-xs">{item.complexity}</div></div></div></div>;
}